...
if not df_f.empty:
    ...
    col2.metric("Shops", df_f["shop_name"].nunique() if "shop_name" in df_f.columns else 0)
    col3.metric("Avg price", f"{df_f['price'].mean():.2f}" if "price" in df_f.columns else "—")
    col4.metric("Avg rating", f"{df_f['rating'].mean():.2f}" if "rating" in df_f.columns else "—")
else:
...
if not df_f.empty and "shop_name" in df_f.columns and "score" in df_f.columns:
    shop_scores = df_f.groupby("shop_name").agg({"score": "mean", "product_id": "count"}).reset_index()
    shop_scores.columns = ["Shop", "Avg score", "Product count"]
    st.dataframe(shop_scores.sort_values("Avg score", ascending=False), use_container_width=True)
else:
...
if not pca.empty and "cluster" in pca.columns:
    fig = px.scatter(pca, x="pc1", y="pc2", color="cluster", title="PCA 2D projection by cluster")
